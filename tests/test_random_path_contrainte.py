...
import popularMatching, randomPathPopWithContrainte, utility, allocTools
import os
...
def test_random_path(nbTests, f, nb_player, listPreferenceAvecCycle, max):
    ...
    while nbDone < nbTests:
        ...
        if rep:
            ...
            repRand, nbC = randomPathPopWithContrainte.random_path_to_popular(init, preferences, f, True)
            if repRand:
    ...
    print(
        "nbInstances {} / nbPopFound {} /  nb nonPop ajoute contrainte toujour non pop {} / nbNOTProp {} / nb qui est populaire mais ne peut pas prendere bon resultat apres ajouter une contrainte {} / nonPropWithCycles {}  /cpt{}".format(
            nbDone, nbPopFound, nbInstCycle, propNotPopular, nbNotPopFound, nbInstCycleNotProp,cpt))
    ...
    return (nbDone, nbPopFound, nbInstCycle, propPopular / (propPopular + propNotPopular),
            propNotPopular / (propPopular + propNotPopular), nbNotPopFound, propCycles,listWrong)
...
def main():
    ...
    listPreferenceAvecCycle = [[" " for i in range(3317)] for j in range(max)]
    ...
    for i in range(4, max):
        ...
        print(
            "nb agents : {} nbInstances {} / nbPopFound {} where  nb Instances with cycles {} /  propPop {} / propNotPop {} /  nb Non prop not found {} with {} cycles"
