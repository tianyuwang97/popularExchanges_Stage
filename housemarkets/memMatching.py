...
def eliminate_agentRandom(status_agents, FH, SH, FA,  SA, nb_players):
    ...
    if len(pickList)==0:
        ...
        agentPick=random.randint(0, len(pickList)-1)
...
def mem_matching(nb_resources ,utilities, u, display=False):
    ...
    while (notEnd) :
        ...
        #STEP 1
            ...
            print("STEP 3")
            if(eliminate_agent(status_agents, FH, SH, FA, SA, nb_resources )): #on donne le preference pour choisir l'agent pour eliminer.
                step = 1
            else :
                step = 4
            if (display):
...
def main():
    ...
    preferences = [1, 3, 2, 4], [1, 3, 2, 4], [2, 4, 1, 3], [1, 2, 3, 4]  #
    #preferences = [[4, 2, 1, 3], [2, 3, 1, 4], [4, 2, 3, 1], [3, 4, 1, 2]]
    preferences = [[1, 5, 3, 2, 4], [5, 4, 1, 3, 2], [5, 4, 2, 3, 1], [3, 5, 1, 2, 4], [5, 3, 2, 4, 1]]

    ScoreBorda,RangMaxMin=mem_matching(5, preferences,utility.additive, True)
